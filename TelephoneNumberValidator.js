...
function telephoneCheck(str) {
  // Define la expresión regular para validar el formato del número de teléfono
  var regex = /^(1\s?)?(\(\d{3}\)|\d{3})([\s\-]?)\d{3}([\s\-]?)\d{4}$/;

  // Comprueba si la cadena cumple con la expresión regular
  return regex.test(str);
}

// Ejemplo de uso
console.log(telephoneCheck("555-555-5555")); // Devuelve true
